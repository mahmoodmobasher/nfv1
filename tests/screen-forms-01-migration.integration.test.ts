...
const head = "1787782332432", ownWhen = "1787779688388";
...
suite("SCREEN-FORMS-01 migration integrity", () => {
  ...
  it("applies its own migration once from fresh state and reruns as a no-op", async () => {
    ...
    expect((await db.query("select count(*)::int count from drizzle.__drizzle_migrations where created_at=$1",[ownWhen])).rows[0]).toEqual({ count: 1 });
    await db.end();
    await migrate(url);
