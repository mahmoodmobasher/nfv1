...
const platformFiles = files("src/backend/platform").filter((path) =>
  path.endsWith(".ts"),
);
...
function routeViolation(source: string) {
  return /(?:@\/backend\/modules\/.+\/persistence\/|(?:\.\.\/)+.*(?:persistence|repositories)\/|@\/server\/db)/.test(
    source,
  );
}
function clientServerViolation(source: string) {
  return (
    ...
    /from\s+["']@\/(?:backend|server)\//.test(source)
  );
}
...
function ownershipRows(markdown: string) {
  const section =
    markdown
      .split("## Table ownership inventory")[1]
      ?.split("## Stable identity inventory")[0] ?? "";
  return [...section.matchAll(/^\|\s*`([^`]+)`\s*\|\s*([^|]+)\|/gm)].map(
    (match) => ({ table: match[1], owner: match[2].trim() }),
  );
}
...
function sqlTables(source: string) {
  ...
  return [
    ...source.matchAll(
      /\b(?:from|join|insert\s+into|update\s+(?!of\b)|delete\s+from)\s*([a-z][a-z0-9_]*)/gi,
    ),
  ]
    .map((match) => match[1].toLowerCase())
...
function sqlOwnershipViolations(path: string, source: string) {
  ...
  const reviewed = path.endsWith(
    "companies/application/read-models/contact-company-candidate.read-model.ts",
  )
    ? new Set(["companies", "contacts"])
    : (ownedByModule[moduleName] ?? new Set<string>());
...
const platformSql = {
  ...
  audit: new Set(["audit_events"]),
  outbox: new Set(["outbox_messages"]),
...
describe("P1A modular-monolith boundaries", () => {
  it("declares public entries, manifests, Platform inventory, and every P1A table exactly once", () => {
    for (const moduleName of modules) {
      expect(
        statSync(`src/backend/modules/${moduleName}/index.ts`).isFile(),
      ).toBe(true);
      expect(
        statSync(`src/backend/modules/${moduleName}/README.md`).isFile(),
      ).toBe(true);
    }
    ...
    for (const area of [
      ...
      expect(
        statSync(`src/backend/platform/${area}/index.ts`).isFile(),
        area,
      ).toBe(true);
      expect(
        readFileSync("src/backend/platform/README.md", "utf8").toLowerCase(),
      ).toContain(area);
    }
    const rows = ownershipRows(registry),
      required = [
        "leads",
        "lead_intakes",
        "lead_activities",
  ...
  it("blocks private cross-module, route-repository, and client-server imports", () => {
    for (const path of files("src/app/api/workspaces/[workspaceId]").filter(
      (item) =>
        item.endsWith("route.ts") && /\/(?:leads|pipeline-stages)\//.test(item),
    ))
    ...
    for (const path of moduleFiles) {
      ...
      const privateImport = [
        ...source.matchAll(
          /@\/backend\/modules\/([^/]+)\/(domain|application|persistence|policies|events|testing)\//g,
        ),
      ].find((match) => match[1] !== own);
      expect(privateImport, path).toBeUndefined();
    }
    for (const path of files("src").filter((item) =>
      /\.(?:ts|tsx)$/.test(item),
    ))
      expect(clientServerViolation(readFileSync(path, "utf8")), path).toBe(
        false,
      );
    expect(
      routeViolation(
        `import {x} from "../../backend/modules/leads/persistence/repositories/x"`,
      ),
    ).toBe(true);
    expect(routeViolation(`import {db} from "@/server/db/client"`)).toBe(true);
    expect(
      clientServerViolation(
        `"use client"; import {x} from "@/backend/modules/leads"`,
      ),
    ).toBe(true);
  });
  ...
  it("rejects undeclared and cross-owner SQL, including negative fixtures", () => {
    for (const path of moduleFiles)
      expect(
        sqlOwnershipViolations(path, readFileSync(path, "utf8")),
        path,
      ).toEqual([]);
    for (const path of platformFiles)
      expect(
        platformSqlViolations(path, readFileSync(path, "utf8")),
        path,
      ).toEqual([]);
    expect(
      sqlOwnershipViolations(
        "src/backend/modules/contacts/persistence/x.ts",
        "select * from companies",
      ),
    ).toEqual(["companies"]);
    expect(
      sqlOwnershipViolations(
        "src/backend/modules/leads/persistence/x.ts",
        "insert into mystery_table(id) values(1)",
      ),
    ).toEqual(["mystery_table"]);
    expect(
      sqlOwnershipViolations(
        "src/backend/modules/companies/application/read-models/contact-company-candidate.read-model.ts",
        "select * from contacts join companies on true",
      ),
    ).toEqual([]);
    expect(
      sqlOwnershipViolations(
        "src/backend/modules/identity-review/persistence/x.ts",
        "with selected as materialized (select * from lead_identity_reviews) select * from selected",
      ),
    ).toEqual([]);
    expect(
      platformSqlViolations(
        "src/backend/platform/audit/x.ts",
  ...
  it("has an acyclic public module graph and proves the negative cycle fixture", () => {
    const graph = Object.fromEntries(
      [...new Set(moduleFiles.map((path) => path.split("/")[3]))].map(
    ...
    for (const path of moduleFiles) {
      ...
      for (const match of source.matchAll(/@\/backend\/modules\/([^/]+)["']/g))
        if (match[1] !== own) graph[own].push(match[1]);
    }
  ...
  it("keeps stable operations/events unique, registered, and fixture-locked", () => {
    const fixture = JSON.parse(
      readFileSync("tests/fixtures/p1a-contract-v1.json", "utf8"),
    ) as Record<string, string[]>;
    const stableSection =
      registry.split("## Stable identity inventory")[1] ?? "";
    const identities = [
      ...stableSection.matchAll(
        /^\|\s*(?:operation|query|Audit|event)\s*\|\s*`([^`]+)`\s*\|/gm,
      ),
    ].map((match) => match[1]);
  ...
  it("requires public operations in manifests/registry and forbids wildcard repository exports", () => {
    const leadManifest = readFileSync(
      "src/backend/modules/leads/README.md",
      "utf8",
    );
    for (const operation of [
      ...
      "listLeadSummariesV1",
      "getLeadDetailV1",
      "listLeadPipelineStagesV1",
      "getLeadOperationalEditV1",
      "editLeadOperationalV1",
      "transitionLeadStageV1",
      "getIdentityReviewDetailV1",
      "listIdentityReviewQueueV1",
      "decideLeadIdentityReviewV1",
    ])
      expect(`${registry}\n${leadManifest}`).toContain(operation);
    for (const moduleName of modules)
      expect(
        readFileSync(`src/backend/modules/${moduleName}/index.ts`, "utf8"),
      ).not.toMatch(/export \* /);
    expect("manifest without operation".includes("submitLeadInquiryV1")).toBe(
      false,
    );
  });
});
