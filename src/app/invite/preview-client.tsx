...
export function InvitationPreview({ plan }: { plan: PlanKey; canOpenOperationalInvitations: boolean }) {
  ...
  const [input, setInput] = useState(""), [invites, setInvites] = useState<PreviewInvite[]>([]), [error, setError] = useState(""), [state, setState] = useState<"idle" | "success" | "partial" | "failed">("idle");
  function addEmails(value: string) {
    ...
    if (entries.length) { setInvites(current => [...current, ...entries.map(email => ({ email, role: "Member" as const, status: "pending" as const }))]); setError(""); setInput(""); }
  }
  ...
  return <div className="onboarding-page"><main className="onboarding-narrow"><section className="flow-card"><p className="eyebrow">Non-persistent demonstration</p><h1>Preview team invitations</h1><p className="lead">Try the invitation form and its recovery states without contacting anyone or changing Workspace access.</p><Alert><b>Preview only.</b> Nothing on this page sends email, reserves seats, creates Memberships, assigns Roles, or writes Audit events.</Alert><p className="helper"><b>{item.name}</b> capacity is server-authoritative. Pending preview entries do not reserve a real seat.</p>{(state === "success" || state === "partial") && <Alert kind="success"><b>{state === "partial" ? "Partial-result preview." : "Success-result preview."}</b> This preview did not send email or create Memberships.</Alert>}{error && <Alert kind="error">{error}</Alert>}<form onSubmit={submit} noValidate><label className="field" htmlFor="invite-email"><span>Work email<em> Required</em></span><span className="input-wrap"><input id="invite-email" value={input} onChange={event => setInput(event.target.value)} placeholder="alex@example.com" type="email" autoComplete="email" /></span></label><button type="button" className="secondary add-button" onClick={() => addEmails(input)}>Add preview entry</button><button className="primary">Preview invitation result</button></form></section></main></div>;
}
