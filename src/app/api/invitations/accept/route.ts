...
export async function POST(request:Request){
  const blocked=await auditedMutationGuard(request,{action:"workspace.invitation_accept_denied",targetType:"invitation"});if(blocked)return blocked;
  ...
  try{
    ...
    return success(await acceptInvitation(pool,{...actor,token:parsed.data.token,idempotencyKey:key,secret:env.SESSION_SECRET}));
  }catch(error){return serviceOwnsDenial?failure(error):auditedFailure(pool,request,error,{action:"workspace.invitation_accept_denied",targetType:"invitation"})}
