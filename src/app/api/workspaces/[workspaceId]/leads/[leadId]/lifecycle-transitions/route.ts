import { LeadManagementError, leadLifecycleTransitionCommandV1Schema, leadManagementFailure, leadManagementJson,
  LEAD_LIFECYCLE_TRANSITION_OPERATION, transitionLeadLifecycleV1 } from "@/backend/modules/leads";
...
export async function POST(request: Request, { params }: { params: Promise<{ workspaceId: string; leadId: string }> }) {
  ...
  try {
    ...
    if (!raw || typeof raw !== "object" ||
      (raw as { contractVersion?: unknown }).contractVersion !== LEAD_LIFECYCLE_TRANSITION_OPERATION)
      throw new LeadManagementError("unsupported_contract_version", 400);
    const parsed = leadLifecycleTransitionCommandV1Schema.safeParse(raw);
    ...
    return leadManagementJson(await transitionLeadLifecycleV1(pool, { actor, leadId, command: parsed.data,
