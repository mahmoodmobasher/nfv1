import { LeadManagementError, leadManagementFailure, leadManagementJson, leadStageTransitionCommandV1Schema,
  transitionLeadStageV1 } from "@/backend/modules/leads";
...
export async function POST(request: Request, { params }: { params: Promise<{ workspaceId: string; leadId: string }> }) {
  ...
  try {
    ...
    if (!raw || typeof raw !== "object" || (raw as { contractVersion?: unknown }).contractVersion !== "lead-stage-transition.v1")
      throw new LeadManagementError("unsupported_contract_version", 400);
    const parsed = leadStageTransitionCommandV1Schema.safeParse(raw);
    ...
    return leadManagementJson(await transitionLeadStageV1(pool, { actor, leadId, command: parsed.data,
