  ...
  return (
    <WebsiteShell action="help">
      <Shell step={4} authLink={false}>
        ...
        <h1>Your company Workspace is ready</h1>
        <p className="mt-2 max-w-3xl text-[13px] leading-6 text-ink-muted">
          {workspace.name} is active and selected for this session.
        </p>
        <div className="grid gap-px overflow-hidden rounded-control border border-line bg-line [&_p]:grid [&_p]:grid-cols-2 [&_p]:gap-4 [&_p]:bg-surface-muted [&_p]:p-3 [&_span]:text-ink-muted">
          ...
          <p>
            ...
            <b>{facts.active_seats} (Owner included)</b>
          </p>
          <p>
            <span>Your role</span>
            <b>Owner</b>
        ...
        <p className="rounded-control border border-line bg-surface-muted p-4 text-sm leading-6 text-ink-muted">
          You are the sole initial Owner. You control subscription, ownership
          and Workspace governance. Invitations are optional; accepted active
          Admins or Members use{" "}
          {additionalSeats
            ? `one of the ${additionalSeats} remaining seats`
            : "an available plan seat"}
          .
        </p>
        <Link className="inline-flex min-h-11 items-center justify-center rounded-control border border-accent bg-accent px-4 text-sm font-semibold text-on-accent hover:bg-accent-ink" href="/crm/leads/new">
          Add your first lead
        </Link>
        <Link
