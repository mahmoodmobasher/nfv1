...
export default async function Page({ params }: { params: Promise<{ dealId: string }> }) { const { dealId } = await params, { pool, workspace, context } = await crmPageContext(`/crm/deals/${dealId}`); try { return <CrmShell workspace={workspace.name} role={context.role}><section className="admin-content narrow-admin"><DealDetailPage workspaceId={workspace.id} dealId={dealId}/></section></CrmShell>; } finally { await pool.end(); } }
