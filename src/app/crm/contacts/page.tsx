...
export default async function Page() { const { pool, workspace, context } = await crmPageContext("/crm/contacts"); try { return <CrmShell workspace={workspace.name} role={context.role}><section className="mx-auto grid w-full max-w-[1400px] gap-5 py-5"><CustomerGraphListPage workspaceId={workspace.id} kind="contact"/></section></CrmShell>; } finally { await pool.end(); } }
