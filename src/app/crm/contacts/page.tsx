...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) { const query = await searchParams, status = query.status === "archived" ? "archived" : "active", cursor = typeof query.cursor === "string" ? query.cursor : undefined, { pool, workspace, context } = await crmPageContext("/crm/contacts"); try { return <CrmShell workspace={workspace.name} role={context.role}><section className="admin-content"><CustomerGraphListPage workspaceId={workspace.id} kind="contact" initialStatus={status} initialCursor={cursor}/></section></CrmShell>; } finally { await pool.end(); } }
