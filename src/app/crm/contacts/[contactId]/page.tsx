...
export default async function Page({ params }: { params: Promise<{ contactId: string }> }) { const { contactId } = await params, { pool, workspace, context } = await crmPageContext(`/crm/contacts/${contactId}`); try { return <CrmShell workspace={workspace.name} role={context.role}><section className="mx-auto grid w-full max-w-[1400px] gap-5 py-5"><CustomerGraphDetailPage workspaceId={workspace.id} kind="contact" id={contactId}/></section></CrmShell>; } finally { await pool.end(); } }
