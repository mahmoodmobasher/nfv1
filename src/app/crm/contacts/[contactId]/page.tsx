...
export default async function Page({ params }: { params: Promise<{ contactId: string }> }) { const { contactId } = await params, { pool, workspace, context } = await crmPageContext(`/crm/contacts/${contactId}`); try { return <CrmShell workspace={workspace.name} role={context.role}><section className="admin-content narrow-admin"><CustomerGraphDetailPage workspaceId={workspace.id} kind="contact" id={contactId}/></section></CrmShell>; } finally { await pool.end(); } }
