...
export const metadata = { title: "Edit Lead operations | NexaFlow" };
...
export default async function Page({ params }: { params: Promise<{ leadId: string }> }) {
  ...
  try {
    try {
      ...
      return <CrmShell workspace={workspace.name} role={context.role}><section className="admin-content narrow-admin p1a-feature"><header className="product-page-header"><div><p className="eyebrow">Leads / Edit</p><h1>Edit Lead operations</h1><p className="lead">Update responsibility and visibility. Identity-bearing corrections are not available in this MVP, and attribution, lifecycle and Pipeline stage remain unchanged.</p></div></header>{view.capabilities.canEditLead ? <LeadOperationalEditForm workspaceId={workspace.id} initial={view}/> : <FeedbackState tone="warning" title="Editing isn’t available" action={<Link className="ds-action ds-action--secondary" href={`/crm/leads/${leadId}`}>Return to Lead details</Link>}><p>Only authorized Workspace Owners and Admins can edit Lead operations.</p></FeedbackState>}</section></CrmShell>;
    } catch (error) {
