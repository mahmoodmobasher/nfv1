...
export default async function Page(){const{pool,workspace,context}=await crmPageContext("/crm/leads/new");try{const[stages,owners,teams]=await Promise.all([pipelineStages(pool,context),leadOwners(pool,context),leadTeams(pool,context)]);return <CrmShell workspace={workspace.name} role={context.role}><section className="admin-content narrow-admin"><header className="product-page-header"><div><p className="eyebrow">Leads / New</p><h1>Add a lead</h1><p className="lead">Create a persistent customer record for your workspace.</p></div></header><LeadEditor workspaceId={workspace.id} stages={stages} owners={owners} teams={teams}/></section></CrmShell>}finally{await pool.end()}}
