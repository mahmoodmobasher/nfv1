...
- Routes: no independent route authority. Leads owns the thin identity-review detail/queue GET and decision POST operations and consumes Identity Review public participants.
- Read trace: Leads protected query -> preview actor/reference -> Leads-owned Lead/intake lock -> Identity Review review/head lock and bounded evidence -> Company/Contact public target locks -> Platform assignment/full-actor authority fence -> strict bounded response.
- Queue participant: Workspace/state/index-keyset reads and evidence counts use only Identity Review-owned tables; Lead context and visibility remain Leads/Platform public participants.
