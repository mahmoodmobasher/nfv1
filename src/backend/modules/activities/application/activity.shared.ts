import { ACTIVITY_SUPPORTED_VERSION, ActivityError, type ActivityErrorCode, type ActivityItemV1 }
...
export function activityItem(row: ActivityRow): ActivityItemV1 {
  if (row.version !== ACTIVITY_SUPPORTED_VERSION) activityFail("activity_unavailable", 503);
  return { activityId: row.activity_id, version: row.version, target: { recordType: "crm.lead", recordId: row.record_id },
