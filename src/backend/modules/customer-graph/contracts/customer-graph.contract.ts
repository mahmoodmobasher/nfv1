...
export const customerGraphListQueryV1Schema = z.object({ status: z.enum(["active","archived"]).default("active"),
  cursor: z.string().max(1024).optional(), limit: z.number().int().min(1).max(50).default(25) }).strict();
...
export const customerGraphListViewV1Schema=z.object({contractVersion:z.literal("customer-graph-list.v1"),kind:z.enum(["company","contact"]),items:z.array(z.object({id:uuid,displayName:clean(200),status:z.enum(["active","archived"]),version:expectedVersion,updatedAt:z.string().datetime({offset:true}),capabilities,reconciliation}).strict()).max(50),nextCursor:z.string().max(1024).nullable(),requestId:uuid}).strict();
