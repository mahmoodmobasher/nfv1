...
export function LeadSummaryCard({ lead, management }: { lead: LeadSummaryItem; management?: { workspaceId: string; stages: LeadPipelineStage[] } }) {
  return <article className="grid gap-2 rounded-card border border-line bg-surface p-3" data-lead-id={lead.leadId}><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><span className="grid gap-1 rounded-control border border-line bg-surface-muted p-3"><span className="text-[10.5px] font-bold uppercase tracking-[.08em] text-ink-faint">Lead lifecycle</span><span className="font-semibold text-ink"><StatusBadge tone={lifecycleTone(lead.lifecycle.status)}>{lead.lifecycle.label ?? lead.lifecycle.status}</StatusBadge></span></span><span className="grid gap-1 rounded-control border border-line bg-surface-muted p-3"><span className="text-[10.5px] font-bold uppercase tracking-[.08em] text-ink-faint">Pipeline stage</span><span className="font-semibold text-ink">{lead.stage.name}</span></span></div><h3><Link href={`/crm/leads/${lead.leadId}`}>{lead.displayName}</Link></h3><p>{lead.company.displayName ?? "No company"}</p><p>{leadContactLabel(lead)}</p><small>{leadAssignmentLabel(lead)} · {reviewLabel(lead.identityReviewStatus)}</small><LeadActions lead={lead} compact management={management}/></article>;
}
