...
export function LeadSummaryCard({ lead, management }: { lead: LeadSummaryItem; management?: { workspaceId: string; stages: LeadPipelineStage[] } }) {
  return <article className="ds-lead-card" data-lead-id={lead.leadId}><div className="ds-lead-card__state-grid"><span className="ds-lead-state"><span className="ds-lead-state__label">Lead lifecycle</span><span className="ds-lead-state__value"><StatusBadge tone={lifecycleTone(lead.lifecycle.status)}>{lead.lifecycle.label ?? lead.lifecycle.status}</StatusBadge></span></span><span className="ds-lead-state"><span className="ds-lead-state__label">Pipeline stage</span><span className="ds-lead-state__value">{lead.stage.name}</span></span></div><h3><Link href={`/crm/leads/${lead.leadId}`}>{lead.displayName}</Link></h3><p>{lead.company.displayName ?? "No company"}</p><p>{leadContactLabel(lead)}</p><small>{leadAssignmentLabel(lead)} · {reviewLabel(lead.identityReviewStatus)}</small><LeadActions lead={lead} compact management={management}/></article>;
}
...
export function LeadReadOnlyDetail({ lead, workspaceId, stages = [] }: { lead: LeadSummaryItem; workspaceId?: string; stages?: LeadPipelineStage[] }) {
  ...
  const summary = <><RecordIdentity marker={lead.displayName.split(/\s+/).slice(0,2).map(value => value[0]).join("").toUpperCase()} title={lead.displayName} secondary={leadContactLabel(lead)} meta={<span className="ds-lead-state"><span className="ds-lead-state__label">Lead lifecycle</span><span className="ds-lead-state__value"><StatusBadge tone={lifecycleTone(lead.lifecycle.status)}>{lead.lifecycle.label ?? lead.lifecycle.status}</StatusBadge></span></span>}/><div className="ds-fact-list"><RelationshipRow label="Company" value={lead.company.displayName ?? "No company provided"}/><RelationshipRow label="Pipeline stage" value={lead.stage.name}/><RelationshipRow label="Responsibility" value={leadAssignmentLabel(lead)}/><RelationshipRow label="Identity review" value={reviewLabel(lead.identityReviewStatus)}/></div></>;
