...
function LeadActions({ lead, compact = false, management }: { lead: LeadSummaryItem; compact?: boolean; management?: { workspaceId: string; stages: LeadPipelineStage[] } }) {
  return <div className="ds-lead-actions"><ActionLink variant="secondary" href={`/crm/leads/${lead.leadId}`}>View lead</ActionLink>{management && lead.capabilities.canMoveStage && <LeadStageMove workspaceId={management.workspaceId} leadId={lead.leadId} leadName={lead.displayName} version={lead.version} currentStageId={lead.stage.id} currentStageName={lead.stage.name} stages={management.stages} focusDestinationOnSuccess/>} {lead.capabilities.canReview && lead.nextView.kind === "identity_review_detail" && <ActionLink variant="primary" href={`/crm/identity-reviews/${lead.leadId}`}>{compact ? "Review" : "Continue review"}</ActionLink>}</div>;
}
...
export function LeadPipeline({ stages, workspaceId }: { stages: Array<{ stage: LeadPipelineStage; view: LeadSummariesView }>; workspaceId?: string }) {
  ...
  return <section className="ds-pipeline" aria-labelledby="pipeline-board-title"><h2 id="pipeline-board-title" className="sr-only">Pipeline board</h2><p role="status">{total} {total === 1 ? "lead" : "leads"} shown across {stages.length} stages.</p><div className="pipeline-board">{stages.map(({ stage, view }) => <section className="pipeline-stage ds-stage-column" key={stage.stageId} aria-labelledby={`stage-${stage.stageId}`}><header><h2 id={`stage-${stage.stageId}`} tabIndex={-1}>{stage.name}</h2><StatusBadge>{view.items.length} {view.items.length === 1 ? "lead" : "leads"}</StatusBadge></header>{view.items.length === 0 ? <p className="helper pipeline-empty-stage">No leads in this stage.</p> : <div className="ds-pipeline__cards">{view.items.map(lead => <LeadSummaryCard key={lead.leadId} lead={lead} management={workspaceId ? {workspaceId,stages:registry} : undefined}/>)}</div>}</section>)}</div></section>;
}
...
export function LeadReadOnlyDetail({ lead, workspaceId, stages = [] }: { lead: LeadSummaryItem; workspaceId?: string; stages?: LeadPipelineStage[] }) {
  ...
  return <><header className="product-page-header ds-page-header"><div><p className="eyebrow">Leads / Details</p><h1>{lead.displayName}</h1><p className="lead">Confirmed Lead information. Identity, attribution and lifecycle values remain protected from ordinary editing.</p></div><StatusBadge tone={lifecycleTone(lead.lifecycle.status)}>{lead.lifecycle.label ?? lead.lifecycle.status}</StatusBadge></header><div className="ds-detail-layout"><Panel tone="overview" title="Contact and Company" description="Optional information is shown only when present.">{facts(contact)}</Panel><Panel tone="access" title="Pipeline and responsibility">{facts(pipeline)}</Panel><Panel tone="relationship" title="Original attribution" description="Source and intake channel remain separate and read-only.">{facts(attribution)}</Panel></div><div className="ds-page-actions"><ActionLink href="/crm">Back to leads</ActionLink><ActionLink href="/crm/pipeline">View pipeline</ActionLink>{lead.capabilities.canEditLead && <ActionLink variant="primary" href={`/crm/leads/${lead.leadId}/edit`}>Edit lead</ActionLink>}{workspaceId && lead.capabilities.canMoveStage && <LeadStageMove workspaceId={workspaceId} leadId={lead.leadId} leadName={lead.displayName} version={lead.version} currentStageId={lead.stage.id} currentStageName={lead.stage.name} stages={stages}/>} {lead.capabilities.canReview && lead.nextView.kind === "identity_review_detail" && <ActionLink variant="primary" href={`/crm/identity-reviews/${lead.leadId}`}>Continue identity review</ActionLink>}</div></>;
}
