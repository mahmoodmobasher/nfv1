...
function ConversionDialog({ titleId, descriptionId, restoreFocus, onClose, children }: { titleId: string; descriptionId: string; restoreFocus: () => void; onClose: () => void; children: ReactNode }) {
  ...
  return <dialog ref={dialog} className="m-auto w-[min(36rem,calc(100%-2rem))] rounded-card border border-line bg-surface p-5 text-ink shadow-e3 backdrop:bg-blanket" aria-labelledby={titleId} aria-describedby={descriptionId} onCancel={event => { event.preventDefault(); onClose(); }}>{children}</dialog>;
}
...
function LeadConversionPanel({ workspaceId, leadId, leadVersion, onAuthorityLoss }: { workspaceId: string; leadId: string; leadVersion: number; onAuthorityLoss: (error: LeadConversionError) => void }) {
  ...
  return <Panel tone="conversion" title="Convert Lead to Deal" description="Only the current server preview authorizes this action. Review every destination and atomic effect before confirming.">
    ...
    <form className="grid gap-4" onSubmit={review} noValidate aria-busy={busy}>
      ...
      <div className="flex flex-wrap items-center gap-2"><button ref={trigger} type="submit" className="inline-flex min-h-11 items-center justify-center rounded-control border border-accent bg-accent px-3.5 py-2 text-[12.5px] font-semibold text-on-accent hover:bg-accent-ink disabled:opacity-45" disabled={busy || stale}>Review conversion</button></div>
    </form>
    {confirming && pendingCommand && <ConversionDialog titleId="lead-conversion-title" descriptionId="lead-conversion-description" restoreFocus={() => trigger.current?.focus()} onClose={() => setConfirming(false)}><h2 id="lead-conversion-title">Convert Lead to Deal?</h2><p id="lead-conversion-description">This atomic operation will create one Deal, link only the selected existing customer records, record lineage, and mark the canonical Lead converted.</p><dl className="grid gap-2 [&_div]:grid [&_div]:grid-cols-[minmax(8rem,auto)_1fr] [&_div]:gap-3 [&_dt]:text-ink-muted [&_dd]:m-0 [&_dd]:font-semibold [&_dd]:text-ink"><div><dt>Deal</dt><dd>{pendingCommand.deal.name}</dd></div><div><dt>Company</dt><dd>{company?.label ?? "Unavailable"}</dd></div><div><dt>Primary Contact</dt><dd>{contact?.label ?? "None"}</dd></div><div><dt>Pipeline and stage</dt><dd>{preview.pipeline!.label} · {preview.pipeline!.initialStage.label}</dd></div></dl><div className="flex flex-wrap items-center gap-2"><Button onClick={() => setConfirming(false)} disabled={busy}>Cancel</Button><Button variant="primary" onClick={() => void convert()} disabled={busy}>{busy ? "Converting…" : "Convert Lead to Deal"}</Button></div></ConversionDialog>}
  </Panel>;
}
