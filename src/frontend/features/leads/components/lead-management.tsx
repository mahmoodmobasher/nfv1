...
function ManagementDialog({ titleId, descriptionId, initialFocus = "cancel", restoreFocus, onClose, children }: {
  ...
  return <dialog ref={dialog} className="m-auto w-[min(36rem,calc(100%-2rem))] rounded-card border border-line bg-surface p-5 text-ink shadow-e3 backdrop:bg-blanket" aria-labelledby={titleId} aria-describedby={descriptionId}
...
export function LeadOperationalEditForm({ workspaceId, initial }: { workspaceId: string; initial: LeadOperationalEditView }) {
  const [view, setView] = useState(initial), [membership, setMembership] = useState(initial.operational.responsibleMembershipId ?? ""),
  ...
  if (authorityLost || !view.capabilities.canEditLead) return <FeedbackState tone="danger" autoFocus title="Editing is no longer available"
    action={<Link className="inline-flex min-h-11 items-center justify-center rounded-control border border-control bg-surface px-3.5 py-2 text-[12.5px] font-semibold text-ink hover:bg-surface-muted disabled:opacity-45" href={`/crm/leads/${view.leadId}`}>Return to Lead details</Link>}><p>Your authority changed or this Lead is no longer available. No operational choices are shown.</p></FeedbackState>;
  if (saved) return <FeedbackState tone="success" autoFocus title={notice} action={<Link className="inline-flex min-h-11 items-center justify-center rounded-control border border-accent bg-accent px-3.5 py-2 text-[12.5px] font-semibold text-on-accent hover:bg-accent-ink disabled:opacity-45" href={`/crm/leads/${view.leadId}`}>Return to Lead details</Link>}>
    <p>Responsibility and visibility now reflect the authoritative saved values.</p></FeedbackState>;
  return <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]"><Panel title="Responsibility and visibility" description="Only operational responsibility and visibility can be changed here. Identity-bearing corrections are not available in this editor.">
    {notice && <div ref={noticeRef} className="grid gap-3 rounded-card border border-line bg-surface-muted p-4 text-ink" role={errors._form ? "alert" : "status"} tabIndex={-1}><div><p>{notice}</p></div>{notice.includes("reload") || notice.includes("changed") ? <div className="flex flex-wrap items-center gap-2"><Button type="button" onClick={() => void reloadLatest()} disabled={busy}>Reload latest</Button></div> : null}</div>}
    {errors._form && <div ref={!notice ? noticeRef : undefined} className="grid gap-3 rounded-card border border-warning bg-warning-soft p-4 text-warning" role="alert" tabIndex={-1}><div><p>{errors._form}</p></div></div>}
    <form className="grid gap-4" onSubmit={submit} noValidate aria-busy={busy}>
      <section><h2>Responsibility</h2><label className="grid min-w-0 gap-1.5 text-xs font-semibold text-ink-muted [&_input]:min-h-11 [&_input]:w-full [&_input]:rounded-control [&_input]:border [&_input]:border-control [&_input]:bg-surface [&_input]:px-3 [&_input]:text-ink [&_select]:min-h-11 [&_select]:w-full [&_select]:rounded-control [&_select]:border [&_select]:border-control [&_select]:bg-surface [&_select]:px-3 [&_select]:text-ink [&_textarea]:min-h-28 [&_textarea]:w-full [&_textarea]:rounded-control [&_textarea]:border [&_textarea]:border-control [&_textarea]:bg-surface [&_textarea]:p-3 [&_textarea]:text-ink" htmlFor="responsibleMembershipId"><span>Responsible person</span><select id="responsibleMembershipId" value={membership} aria-describedby="responsibleMembershipId-help" onChange={event => setMembership(event.target.value)}><option value="">Unassigned</option>{view.options.responsibleMemberships.map(option => <option value={option.id} key={option.id}>{option.label}</option>)}</select><FieldMessage id="responsibleMembershipId-help">Only active people in this Workspace are available.</FieldMessage></label>
      <label className="grid min-w-0 gap-1.5 text-xs font-semibold text-ink-muted [&_input]:min-h-11 [&_input]:w-full [&_input]:rounded-control [&_input]:border [&_input]:border-control [&_input]:bg-surface [&_input]:px-3 [&_input]:text-ink [&_select]:min-h-11 [&_select]:w-full [&_select]:rounded-control [&_select]:border [&_select]:border-control [&_select]:bg-surface [&_select]:px-3 [&_select]:text-ink [&_textarea]:min-h-28 [&_textarea]:w-full [&_textarea]:rounded-control [&_textarea]:border [&_textarea]:border-control [&_textarea]:bg-surface [&_textarea]:p-3 [&_textarea]:text-ink" htmlFor="responsibleTeamId"><span>Responsible Team</span><select id="responsibleTeamId" value={team} aria-describedby="responsibleTeamId-help" onChange={event => chooseResponsibleTeam(event.target.value)}><option value="">No responsible Team</option>{view.options.teams.map(option => <option value={option.id} key={option.id}>{option.label}</option>)}</select><FieldMessage id="responsibleTeamId-help">Assignment may be cleared. A responsible Team is included in Team visibility when selected.</FieldMessage></label></section>
      <section><h2>Visibility</h2><fieldset aria-describedby={`visibility-help${errors.visibleTeamIds ? " visibleTeamIds-error" : ""}`}><legend>Who can view this Lead?</legend><label className="flex min-h-11 items-center gap-2 text-xs text-ink-muted [&_input]:size-4 [&_input]:accent-accent"><input type="radio" name="visibility" checked={visibility === "workspace"} onChange={() => setVisibility("workspace")}/>Everyone with access to this Workspace</label><label className="flex min-h-11 items-center gap-2 text-xs text-ink-muted [&_input]:size-4 [&_input]:accent-accent"><input type="radio" name="visibility" checked={visibility === "teams"} onChange={() => { setVisibility("teams"); if (team) setVisibleTeams(values => values.includes(team) ? values : [...values, team]); }}/>Authorized members of selected Teams</label><FieldMessage id="visibility-help">Workspace visibility may broaden access. Team visibility requires at least one active Team.</FieldMessage></fieldset>
      {visibility === "teams" && <fieldset className="grid gap-2 rounded-card border border-line p-3" aria-describedby={errors.visibleTeamIds ? "visibleTeamIds-error" : "visibleTeamIds-help"} aria-invalid={Boolean(errors.visibleTeamIds)}><legend className="px-1 text-xs font-semibold text-ink">Teams that can view this Lead</legend>{view.options.teams.map(option => <label className="flex min-h-11 items-center gap-2 text-xs text-ink-muted [&_input]:size-4 [&_input]:accent-accent" key={option.id}><input type="checkbox" checked={visibleTeams.includes(option.id)} disabled={option.id === team} onChange={event => setVisibleTeams(values => event.target.checked ? [...values, option.id] : values.filter(id => id !== option.id))}/>{option.label}{option.id === team ? " · Responsible Team" : ""}</label>)}<FieldMessage id="visibleTeamIds-help">Choose one or more active Teams.</FieldMessage>{errors.visibleTeamIds && <FieldMessage id="visibleTeamIds-error" tone="error">{errors.visibleTeamIds}</FieldMessage>}</fieldset>}</section>
      <div className="flex flex-wrap items-center gap-2"><Button variant="primary" disabled={busy || !dirty}>{busy ? "Saving changes…" : "Save changes"}</Button><Link className="inline-flex min-h-11 items-center justify-center rounded-control border border-control bg-surface px-3.5 py-2 text-[12.5px] font-semibold text-ink hover:bg-surface-muted disabled:opacity-45" href={`/crm/leads/${view.leadId}`}>Cancel</Link></div>
    </form></Panel></div>;
}
...
export function LeadStageMove({ workspaceId, leadId, leadName, version, currentStageId, currentStageName, stages, focusDestinationOnSuccess = false }: {
  ...
  if (!targets.length) return <span className="text-xs text-ink-faint">No other active stages</span>;
  const titleId = `move-stage-title-${leadId}`, descriptionId = `move-stage-description-${leadId}`;
  return <><button className="inline-flex min-h-11 items-center justify-center rounded-control border border-control bg-surface px-3.5 py-2 text-[12.5px] font-semibold text-ink hover:bg-surface-muted disabled:opacity-45" type="button" onClick={event => { restoreTrigger.current = true; setReturnFocus(event.currentTarget); setTarget(""); setOpen(true); }} aria-haspopup="dialog">Move stage<span className="sr-only"> for {leadName}</span></button>{open && <ManagementDialog titleId={titleId} descriptionId={descriptionId} initialFocus={focusDestinationOnSuccess ? "select" : "cancel"} restoreFocus={() => { if (restoreTrigger.current) returnFocus?.focus(); }} onClose={close}>
    {result ? <div ref={terminalState} role="status" aria-live="polite" aria-atomic="true" tabIndex={-1}><h2 id={titleId}>{result.changed ? `Stage updated to ${result.stage.name}.` : `Already in ${result.stage.name}.`}</h2><p id={descriptionId}>{result.changed ? result.replayed ? "This stage movement was already applied. No duplicate activity was created." : "The Lead was moved and its activity history was updated." : result.replayed ? "This no-change result was already recorded. The Lead remains unchanged." : "The Lead was already in this stage. No Lead change or activity was created."}</p><div className="flex flex-wrap items-center gap-2"><Button variant="primary" type="button" onClick={finish}>Done</Button><Link className="inline-flex min-h-11 items-center justify-center rounded-control border border-control bg-surface px-3.5 py-2 text-[12.5px] font-semibold text-ink hover:bg-surface-muted disabled:opacity-45" href={`/crm/leads/${leadId}`}>View Lead</Link></div></div> : authorityLoss ? <div ref={terminalState} role="alert" aria-atomic="true" tabIndex={-1}><h2 id={titleId}>Lead no longer available</h2><p id={descriptionId}>Access changed or this Lead is no longer visible. No stage change was applied.</p><div className="flex flex-wrap items-center gap-2"><Button variant="primary" type="button" onClick={() => location.reload()}>Reload safely</Button></div></div> : <><h2 id={titleId}>{selected ? `Move ${leadName} from ${currentStageName} to ${selected.name}?` : `Move ${leadName} to another stage?`}</h2><p id={descriptionId}>Current stage: {currentStageName}. Choose an active Pipeline stage and confirm the movement. Lifecycle and status will not change.</p>{error && <div ref={alert} className="grid gap-3 rounded-card border border-danger bg-danger-soft p-4 text-danger" role="alert" tabIndex={-1}><div><p>{leadManagementErrorDisposition(error) === "refetch_edit" || leadManagementErrorDisposition(error) === "refetch_stage" ? "This Lead or the available stages changed. No movement was applied; reload the latest Pipeline before trying again." : error.message}</p></div></div>}<label className="grid min-w-0 gap-1.5 text-xs font-semibold text-ink-muted [&_input]:min-h-11 [&_input]:w-full [&_input]:rounded-control [&_input]:border [&_input]:border-control [&_input]:bg-surface [&_input]:px-3 [&_input]:text-ink [&_select]:min-h-11 [&_select]:w-full [&_select]:rounded-control [&_select]:border [&_select]:border-control [&_select]:bg-surface [&_select]:px-3 [&_select]:text-ink [&_textarea]:min-h-28 [&_textarea]:w-full [&_textarea]:rounded-control [&_textarea]:border [&_textarea]:border-control [&_textarea]:bg-surface [&_textarea]:p-3 [&_textarea]:text-ink" htmlFor={`move-stage-${leadId}`}><span>Pipeline stage</span><select id={`move-stage-${leadId}`} value={target} disabled={busy || Boolean(forbidden)} onChange={event => { setTarget(event.target.value); setError(null); request.current = { body: "", key: crypto.randomUUID() }; }}><option value="">Choose a stage</option>{targets.map(stage => <option key={stage.stageId} value={stage.stageId}>{stage.name}</option>)}</select></label><div className="flex flex-wrap items-center gap-2"><Button data-dialog-cancel type="button" disabled={busy} onClick={close}>Cancel</Button>{forbidden ? <Button variant="primary" type="button" onClick={() => { close(); router.refresh(); }}>Close and refresh</Button> : error && (leadManagementErrorDisposition(error) === "refetch_edit" || leadManagementErrorDisposition(error) === "refetch_stage") ? <Button variant="primary" type="button" onClick={() => { close(); router.refresh(); }}>Reload latest</Button> : <Button variant="primary" type="button" disabled={busy || !selected} onClick={() => void move()}>{busy ? "Moving Lead…" : selected ? `Move from ${currentStageName} to ${selected.name}` : "Choose a target stage"}</Button>}</div></>}
  </ManagementDialog>}</>;
}
