...
export function LeadActivityPanel({ workspaceId, leadId, onAuthorityLoss }: { workspaceId: string; leadId: string; onAuthorityLoss: (error: AuthorityLoss) => void }) {
  ...
  return <Panel className="lead-activity" title="Activity" description="Manual Lead interactions in newest-first order." action={view.lead.capabilities.canCreateActivity ? <span className="lead-activity-capability">You can log activity</span> : undefined}>
    {(notice || error) && <div ref={summary} data-feedback-kind={feedbackKind} className={`ds-feedback ${feedbackKind === "conflict" ? "ds-feedback--conflict" : feedbackKind === "validation" || feedbackKind === "error" ? "ds-feedback--danger" : feedbackKind === "success" ? "ds-feedback--success" : feedbackKind === "replay" ? "ds-feedback--warning" : "ds-feedback--info"}`} role={feedbackKind === "validation" || feedbackKind === "error" || feedbackKind === "conflict" ? "alert" : "status"} aria-live={feedbackKind === "validation" || feedbackKind === "error" || feedbackKind === "conflict" ? "assertive" : "polite"} aria-atomic="true" tabIndex={-1}><div><p>{notice}</p>{Object.entries(errors).length > 0 && <ul>{Object.entries(errors).map(([field, message]) => message && <li key={field}>{field === "_form" ? message : <a href={`#activity-${field}`} onClick={event => { event.preventDefault(); document.getElementById(`activity-${field}`)?.focus(); }}>{message}</a>}</li>)}</ul>}</div>{stale && <div className="ds-feedback__actions"><Button onClick={() => void load(confirmedKind, undefined, false, true)} disabled={loading}>Load latest Lead and activity</Button></div>}</div>}
    <div className="lead-activity-layout">
      {view.lead.capabilities.canCreateActivity && <form className="ds-form lead-activity-form" onSubmit={submit} noValidate aria-busy={busy}>
        <section aria-labelledby="activity-composer-title"><h3 id="activity-composer-title">Log activity</h3>
          <div className="form-grid lead-activity-primary-fields"><label className="field" htmlFor="activity-kind"><span>Kind</span><select id="activity-kind" value={draft.kind} onChange={event => update("kind", event.target.value as ActivityKindV1)}>{kinds.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label className="field" htmlFor="activity-occurredAt"><span>Occurred at</span><input id="activity-occurredAt" type="datetime-local" value={draft.occurredAt} onChange={event => update("occurredAt", event.target.value)} aria-invalid={Boolean(errors.occurredAt)} aria-describedby={described("occurredAt")}/>{errors.occurredAt && <FieldMessage id="activity-occurredAt-error" tone="error">{errors.occurredAt}</FieldMessage>}</label></div>
      ...
      <section className="lead-activity-timeline" aria-labelledby="activity-timeline-title" aria-busy={loadingOlder || loading}><div className="lead-activity-timeline__header"><div><h3 id="activity-timeline-title">Activity history</h3><p role="status">{view.items.length} loaded · {confirmedKind ? `${kinds.find(([value]) => value === confirmedKind)?.[1]} only` : "All manual kinds"}{requestedKind !== confirmedKind ? " · Applying requested filter…" : ""}</p></div><label className="field" htmlFor="activity-filter"><span>Kind</span><select id="activity-filter" value={requestedKind} onChange={event => { const next = event.target.value as "" | ActivityKindV1; setRequestedKind(next); void load(next); }} disabled={loadingOlder}><option value="">All manual kinds</option>{kinds.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label></div>
        {view.items.length === 0 ? <EmptyState title="No activity yet"><p>{confirmedKind ? "No activity matches this kind." : "Log the first manual interaction when current authority permits it."}</p></EmptyState> : <ol className="lead-activity-list" aria-label="Lead activity history">{view.items.map(item => <ActivityCard item={item} key={item.activityId}/>)}</ol>}
        {view.hasMore && view.nextCursor && <div className="lead-activity-load-older"><Button onClick={() => void load(confirmedKind, view.nextCursor!, true)} disabled={loadingOlder || loading}>{loadingOlder ? "Loading older activity…" : "Load older activity"}</Button></div>}
      </section>
    </div>
  </Panel>;
}
