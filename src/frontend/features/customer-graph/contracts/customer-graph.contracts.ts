...
const assignmentShape = {
  responsibleMembershipId: uuid.nullable().default(null), responsibleTeamId: uuid.nullable().default(null),
  visibility: z.enum(["workspace", "teams"]), visibleTeamIds: z.array(uuid).max(20),
};
function assignmentRules(value: { visibility: "workspace" | "teams"; visibleTeamIds: string[]; responsibleTeamId: string | null }, context: z.RefinementCtx) {
  ...
  if (value.visibility === "workspace" && value.visibleTeamIds.length) context.addIssue({ code: "custom", message: "workspace_visibility_has_teams", path: ["visibleTeamIds"] });
  if (value.visibility === "teams" && !value.visibleTeamIds.length) context.addIssue({ code: "custom", message: "visible_team_required", path: ["visibleTeamIds"] });
...
export const companyCreateCommandV1Schema = z.object({ contractVersion: z.literal(COMPANY_CREATE), displayName: clean(200),
  domain: clean(253).regex(/^([a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)*$/i).nullable(), ...assignmentShape }).strict().superRefine(assignmentRules);
export const companyEditCommandV1Schema = z.object({ contractVersion: z.literal(COMPANY_EDIT), expectedVersion, displayName: clean(200),
  domain: clean(253).regex(/^([a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)*$/i).nullable(), ...assignmentShape }).strict().superRefine(assignmentRules);
export const companyLifecycleCommandV1Schema = z.object({ contractVersion: z.literal(COMPANY_LIFECYCLE), expectedVersion }).strict();
const contactFields = { firstName: clean(100), lastName: clean(100).nullable(), email: z.string().trim().email().max(320).nullable(), phone: clean(50).regex(/^\+[1-9]\d{6,14}$/).nullable() };
const affiliation = z.object({ companyId: uuid, roleCode: z.enum(["employee", "owner", "executive", "decision_maker", "billing", "technical", "advisor", "contractor", "other"]) }).strict();
export const contactCreateCommandV1Schema = z.object({ contractVersion: z.literal(CONTACT_CREATE), ...contactFields, affiliation: affiliation.nullable(), ...assignmentShape }).strict().superRefine(assignmentRules);
export const contactEditCommandV1Schema = z.object({ contractVersion: z.literal(CONTACT_EDIT), expectedVersion, ...contactFields, ...assignmentShape }).strict().superRefine(assignmentRules);
export const contactLifecycleCommandV1Schema = z.object({ contractVersion: z.literal(CONTACT_LIFECYCLE), expectedVersion }).strict();
export const contactAffiliationReplaceCommandV1Schema = z.object({ contractVersion: z.literal(CONTACT_AFFILIATION_REPLACE), expectedVersion, affiliation: affiliation.nullable() }).strict();
export const customerGraphListQueryV1Schema = z.object({ status: z.enum(["active", "archived"]).default("active"), cursor: z.string().max(1024).optional(), limit: z.number().int().min(1).max(50).default(25), bootstrap: z.boolean().default(false) }).strict().superRefine((value, context) => { if (value.bootstrap && value.cursor) context.addIssue({ code: "custom", message: "bootstrap_cursor_forbidden", path: ["cursor"] }); });
...
const affiliationView=z.union([z.object({affiliationId:uuid,companyId:uuid,companyName:clean(200),roleCode:z.enum(["employee","owner","executive","decision_maker","billing","technical","advisor","contractor","other"]),isPrimary:z.boolean(),version:expectedVersion,companyVisible:z.literal(true)}).strict(),z.object({companyUnavailable:z.literal(true)}).strict()]);
const detailCapabilities=capabilities.extend({canManageAffiliations:z.boolean(),canManageAssignment:z.boolean()}).strict();
const companyRecord=root.extend({domain:z.string().max(253).nullable(),disclosure:z.object({domain:z.enum(["full","withheld"])}).strict(),affiliations:z.array(affiliationView).max(0),capabilities:detailCapabilities}).strict();
const contactRecord=root.extend({firstName:clean(100).nullable(),lastName:clean(100).nullable(),email:z.string().email().max(320).nullable(),phone:z.string().max(50).nullable(),maskedEmail:z.string().max(320).nullable(),maskedPhone:z.string().max(50).nullable(),disclosure:z.object({channels:z.enum(["full","masked"])}).strict(),affiliations:z.array(affiliationView).max(20),capabilities:detailCapabilities}).strict();
...
export const customerGraphListEnvelopeSchema = z.object({ data: customerGraphListViewV1Schema }).strict();
export const customerGraphDetailEnvelopeSchema = z.object({ data: customerGraphDetailViewV1Schema }).strict();
export const companyResultEnvelopeSchema = z.object({ data: companyResultV1Schema }).strict();
export const contactResultEnvelopeSchema = z.object({ data: contactResultV1Schema }).strict();

export type CustomerGraphKind = "company" | "contact";
export type CustomerGraphStatus = "active" | "archived";
...
export type CompanyResultV1 = z.infer<typeof companyResultV1Schema>;
export type ContactResultV1 = z.infer<typeof contactResultV1Schema>;
export type CustomerGraphErrorEnvelopeV1 = z.infer<typeof customerGraphErrorEnvelopeV1Schema>;
export type CustomerGraphList = CustomerGraphListViewV1;
export type CustomerGraphDetail = CustomerGraphDetailViewV1;
export type CustomerGraphError = z.infer<typeof customerGraphErrorEnvelopeV1Schema>["error"];
