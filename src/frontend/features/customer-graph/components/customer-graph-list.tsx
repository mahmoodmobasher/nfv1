...
function LifecycleDialog({ item, action, busy, onCancel, onConfirm, restore }: { item: Item; action: "archive" | "restore"; busy: boolean; onCancel: () => void; onConfirm: () => void; restore: () => void }) {
  ...
  return <dialog ref={dialog} className="lead-management-dialog" aria-labelledby="cg-list-dialog-title" aria-describedby="cg-list-dialog-description" onCancel={event => { event.preventDefault(); close(); }}>
    ...
    <div className="ds-page-actions"><Button type="button" onClick={close}>Cancel</Button><Button type="button" variant={action === "archive" ? "danger" : "primary"} onClick={onConfirm} disabled={busy}>{busy ? `${action === "archive" ? "Archiving" : "Restoring"}…` : action === "archive" ? "Archive record" : "Restore record"}</Button></div>
  </dialog>;
}
...
function LifecycleAction({ workspaceId, kind, item, onAuthorityLoss, onApplied, onStale }: { workspaceId: string; kind: CustomerGraphKind; item: Item; onAuthorityLoss: (error: CustomerGraphError) => void; onApplied: (message: string) => void; onStale: (status: CustomerGraphStatus) => Promise<void> }) {
  ...
  return <div className="cg-list-lifecycle">
    ...
    <button ref={trigger} type="button" className={`ds-action ds-action--${action === "archive" ? "danger" : "secondary"}`} disabled={stale} onClick={() => { returnFocus.current = trigger.current?.closest("details")?.querySelector<HTMLElement>("summary") ?? trigger.current; setOpen(true); }}>{action === "archive" ? "Archive" : "Restore"}<span className="sr-only"> {item.displayName}</span></button>
