...
import { ActionLink, ActionMenu, Button, DataTable, DataToolbar, EmptyState, FeedbackState, ProductPageHeader, RecordCard, RecordCards, RecordIdentity, StatusBadge } from "@/frontend/design-system";
import {
  COMPANY_LIFECYCLE,
  CONTACT_LIFECYCLE,
...
export function CustomerGraphListPage({ workspaceId, kind }: { workspaceId: string; kind: CustomerGraphKind }) {
  ...
  return <div className="grid gap-4">
    ...
    <DataToolbar label={`Search loaded ${title(kind).toLowerCase()}`} htmlFor={`${kind}-directory-search`} helper={<p id={`${kind}-directory-search-help`}>Search applies only to the records currently loaded below.</p>} status={<p className="text-xs leading-5 text-ink-muted" role="status" aria-live="polite">{announcement || (search ? `${visible.length} of ${loaded} loaded ${title(kind).toLowerCase()} match this search.` : `${loaded} loaded ${loaded === 1 ? singular(kind).toLowerCase() : title(kind).toLowerCase()} shown.`)}</p>}>
      <form className="flex min-w-0 flex-1 flex-wrap items-end gap-2 [&_input]:min-w-[240px] [&_input]:flex-1" role="search" onSubmit={submitSearch}><input id={`${kind}-directory-search`} type="search" value={searchDraft} onChange={event => setSearchDraft(event.target.value)} maxLength={200} aria-describedby={`${kind}-directory-search-help`}/><Button type="submit">Search</Button>{search && <Button type="button" onClick={() => { setSearchDraft(""); setSearch(""); }}>Clear search</Button>}</form>
