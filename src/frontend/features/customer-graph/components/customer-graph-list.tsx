...
import { ActionLink, ActionMenu, Button, DataTable, DataToolbar, EmptyState, FeedbackState, ProductPageHeader, RecordCard, RecordCards, RecordIdentity, SearchButton, SearchInput, StatusBadge } from "@/frontend/design-system";
import {
  COMPANY_LIFECYCLE,
  CONTACT_LIFECYCLE,
...
function marker(value: string) { return value.split(/\s+/).filter(Boolean).slice(0, 2).map(part => part[0]).join("").toUpperCase(); }
...
function LifecycleDialog({ item, action, busy, onCancel, onConfirm, restore }: { item: Item; action: "archive" | "restore"; busy: boolean; onCancel: () => void; onConfirm: () => void; restore: () => void }) {
  ...
  useEffect(() => { const node = dialog.current; if (!node) return; node.showModal(); node.querySelector<HTMLElement>("button")?.focus(); return () => { if (node.open) node.close(); }; }, []);
  function close() { if (dialog.current?.open) dialog.current.close(); onCancel(); setTimeout(() => restoreRef.current()); }
  return <dialog ref={dialog} className="m-auto w-[min(36rem,calc(100%-2rem))] rounded-card border border-line bg-surface p-5 text-ink shadow-e3 backdrop:bg-blanket" aria-labelledby="cg-list-dialog-title" aria-describedby="cg-list-dialog-description" onCancel={event => { event.preventDefault(); close(); }}>
    ...
    <div className="flex flex-wrap items-center gap-2"><Button type="button" onClick={close}>Cancel</Button><Button type="button" variant={action === "archive" ? "danger" : "primary"} onClick={onConfirm} disabled={busy}>{busy ? `${action === "archive" ? "Archiving" : "Restoring"}…` : action === "archive" ? "Archive record" : "Restore record"}</Button></div>
  </dialog>;
}
...
function LifecycleAction({ workspaceId, kind, item, onAuthorityLoss, onApplied, onStale }: { workspaceId: string; kind: CustomerGraphKind; item: Item; onAuthorityLoss: (error: CustomerGraphError) => void; onApplied: (message: string) => void; onStale: (status: CustomerGraphStatus) => Promise<void> }) {
  ...
  return <div className="flex flex-wrap items-center justify-end gap-2">
    {message && <div ref={alert} className={stale ? "rounded-card border border-warning bg-warning-soft p-3 text-warning" : "rounded-card border border-line bg-surface-muted p-3 text-ink-muted"} role={stale ? "alert" : "status"} tabIndex={-1}><p>{message}</p>{stale && <Button type="button" onClick={async () => { await onStale(item.status); setStale(false); setMessage("Latest loaded records are shown. Review this row before continuing."); requestAnimationFrame(() => alert.current?.focus()); }}>Reload latest</Button>}</div>}
    <button ref={trigger} type="button" className={action === "archive" ? "inline-flex min-h-11 items-center justify-center rounded-control border border-danger bg-danger px-3.5 py-2 text-[12.5px] font-semibold text-on-danger disabled:opacity-45" : "inline-flex min-h-11 items-center justify-center rounded-control border border-control bg-surface px-3.5 py-2 text-[12.5px] font-semibold text-ink hover:bg-surface-muted disabled:opacity-45"} disabled={stale} onClick={() => { returnFocus.current = trigger.current?.closest("details")?.querySelector<HTMLElement>("summary") ?? trigger.current; setOpen(true); }}>{action === "archive" ? "Archive" : "Restore"}<span className="sr-only"> {item.displayName}</span></button>
    {open && <LifecycleDialog item={item} action={action} busy={busy} onCancel={() => setOpen(false)} onConfirm={() => void mutate()} restore={() => returnFocus.current?.focus()}/>}
  </div>;
}
...
export function CustomerGraphListPage({ workspaceId, kind }: { workspaceId: string; kind: CustomerGraphKind }) {
  ...
  return <div className="grid gap-4">
    {kind === "company" ? <ProductPageHeader marker="CO" context="Customer records" title="Companies" description={<p>Manage the Company records visible in this Workspace.</p>} action={canCreate ? <ActionLink variant="primary" href={`${base}/new`}>Add company</ActionLink> : undefined}/> : <ProductPageHeader marker="CT" context="Customer records" title="Contacts" description={<p>Manage the Contact records visible in this Workspace.</p>} action={canCreate ? <ActionLink variant="primary" href={`${base}/new`}>Add contact</ActionLink> : undefined}/>}
    <DataToolbar label={`Search loaded ${title(kind).toLowerCase()}`} htmlFor={`${kind}-directory-search`} helper={<p id={`${kind}-directory-search-help`}>Search applies only to the records currently loaded below.</p>} status={<p className="text-xs leading-5 text-ink-muted" role="status" aria-live="polite">{announcement || (search ? `${visible.length} of ${loaded} loaded ${title(kind).toLowerCase()} match this search.` : `${loaded} loaded ${loaded === 1 ? singular(kind).toLowerCase() : title(kind).toLowerCase()} shown.`)}</p>}>
      <form className="flex min-w-0 flex-1 flex-wrap items-center gap-2" role="search" onSubmit={submitSearch}><SearchInput className="min-w-60 flex-1" id={`${kind}-directory-search`} value={searchDraft} onChange={event => setSearchDraft(event.target.value)} maxLength={200} placeholder={`Search loaded ${title(kind).toLowerCase()}`} aria-describedby={`${kind}-directory-search-help`}/><SearchButton type="submit">Search</SearchButton>{search && <SearchButton type="button" onClick={() => { setSearchDraft(""); setSearch(""); }}>Clear search</SearchButton>}</form>
      <label className="flex min-h-11 items-center gap-2 text-xs text-ink-muted [&_input]:size-4 [&_input]:accent-accent"><input type="checkbox" checked={includeArchived} onChange={event => { const checked = event.target.checked; setIncludeArchived(checked); setSearchDraft(""); setSearch(""); if (checked && !archived.items.length && !archived.loading) void load("archived"); }}/>Include archived</label>
    </DataToolbar>
    {active.loading ? <CustomerGraphLoading label={`Loading active ${title(kind).toLowerCase()}…`}/> : active.error ? <FeedbackState tone="danger" autoFocus title={`Active ${title(kind).toLowerCase()} are temporarily unavailable`} action={<Button onClick={() => void load("active")}>Retry active {title(kind).toLowerCase()}</Button>}><p>{active.error.message} No protected rows are shown.</p></FeedbackState> : active.items.length === 0 && (!includeArchived || archived.items.length === 0) && !archived.loading && !archived.error && !search ? <EmptyState title={`No ${title(kind).toLowerCase()} yet`} action={canCreate ? <ActionLink variant="primary" href={`${base}/new`}>Add {singular(kind).toLowerCase()}</ActionLink> : undefined}><p>{includeArchived ? `No active or archived ${title(kind).toLowerCase()} are currently visible.` : `No active ${title(kind).toLowerCase()} are currently visible.`}</p></EmptyState> : search && visible.length === 0 ? <EmptyState title={`No loaded ${title(kind).toLowerCase()} match your search`} action={<Button onClick={() => { setSearchDraft(""); setSearch(""); }}>Clear search</Button>}><p>Clear the search or load more records. This search does not cover records that have not been loaded.</p></EmptyState> : <section className="grid gap-3" aria-label={`Loaded ${title(kind).toLowerCase()}`}>
      <DataTable caption={`Loaded ${title(kind).toLowerCase()}. Active and archived feeds keep separate server ordering.`}><thead><tr><th scope="col">{singular(kind)}</th><th scope="col">Status</th><th scope="col">Updated</th><th scope="col"><span className="sr-only">Actions</span></th></tr></thead><tbody>{visible.map(item => <tr key={item.id} className={item.status === "archived" ? "is-archived" : undefined}><th scope="row"><RecordIdentity marker={marker(item.displayName)} title={item.displayName} href={`${base}/${item.id}`} secondary={`${singular(kind)} record`}/></th><td><StatusBadge tone={item.status === "active" ? "success" : "neutral"}>{item.status === "active" ? "Active" : "Archived"}</StatusBadge></td><td><time dateTime={item.updatedAt}>{date(item.updatedAt)}</time></td><td><ActionMenu label={`Actions for ${item.displayName}`}><ActionLink href={`${base}/${item.id}`}>View<span className="sr-only"> {item.displayName}</span></ActionLink>{item.capabilities.canEdit && <ActionLink href={`${base}/${item.id}/edit`}>Edit<span className="sr-only"> {item.displayName}</span></ActionLink>}<LifecycleAction workspaceId={workspaceId} kind={kind} item={item} onAuthorityLoss={clearProtected} onApplied={message => { setAnnouncement(message); void load("active"); if (includeArchived) void load("archived"); }} onStale={refresh}/></ActionMenu></td></tr>)}</tbody></DataTable>
      <RecordCards label={`Loaded ${title(kind).toLowerCase()}`}>{visible.map(item => <RecordCard key={item.id} title={item.displayName} href={`${base}/${item.id}`} secondary={<StatusBadge tone={item.status === "active" ? "success" : "neutral"}>{item.status === "active" ? "Active" : "Archived"}</StatusBadge>} facts={[{label:"Updated",value:<time dateTime={item.updatedAt}>{date(item.updatedAt)}</time>}]} actions={<ActionMenu label={`Actions for ${item.displayName}`}><ActionLink href={`${base}/${item.id}`}>View<span className="sr-only"> {item.displayName}</span></ActionLink>{item.capabilities.canEdit && <ActionLink href={`${base}/${item.id}/edit`}>Edit<span className="sr-only"> {item.displayName}</span></ActionLink>}<LifecycleAction workspaceId={workspaceId} kind={kind} item={item} onAuthorityLoss={clearProtected} onApplied={message => { setAnnouncement(message); void load("active"); if (includeArchived) void load("archived"); }} onStale={refresh}/></ActionMenu>}/>)}</RecordCards>
      {includeArchived && archived.loading && <CustomerGraphLoading label={`Loading archived ${title(kind).toLowerCase()}…`}/>}
      {includeArchived && archived.error && <FeedbackState tone="warning" title={`Archived ${title(kind).toLowerCase()} could not be loaded`} action={<button ref={archivedRetry} className="inline-flex min-h-11 items-center justify-center rounded-control border border-control bg-surface px-3.5 py-2 text-[12.5px] font-semibold text-ink hover:bg-surface-muted disabled:opacity-45" type="button" onClick={() => void load("archived")}>Retry archived {title(kind).toLowerCase()}</button>}><p>{archived.error.message} {active.items.length ? "Active records remain available above." : `No active ${title(kind).toLowerCase()} are currently visible; archived visibility is unknown until this request succeeds.`}</p></FeedbackState>}
      <div className="flex flex-wrap justify-end gap-2">{active.nextCursor && <div>{active.moreError && <p role="alert">{active.moreError}</p>}<Button type="button" disabled={active.loadingMore} onClick={() => void load("active", true)}>{active.loadingMore ? "Loading active…" : `Load more active ${title(kind).toLowerCase()}`}</Button></div>}{includeArchived && archived.nextCursor && <div>{archived.moreError && <p role="alert">{archived.moreError}</p>}<Button type="button" disabled={archived.loadingMore} onClick={() => void load("archived", true)}>{archived.loadingMore ? "Loading archived…" : `Load more archived ${title(kind).toLowerCase()}`}</Button></div>}</div>
    </section>}
  </div>;
}
