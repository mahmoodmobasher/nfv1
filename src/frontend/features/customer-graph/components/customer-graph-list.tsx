...
import { ActionLink, Button, DataTable, EmptyState, FeedbackState, ProductPageHeader, RecordCard, RecordCards, StatusBadge } from "@/frontend/design-system";
import {
  COMPANY_LIFECYCLE,
  CONTACT_LIFECYCLE,
...
export function CustomerGraphListPage({ workspaceId, kind }: { workspaceId: string; kind: CustomerGraphKind }) {
  ...
  return <div className="cg-directory">
    ...
    {active.loading ? <CustomerGraphLoading label={`Loading active ${title(kind).toLowerCase()}…`}/> : active.error ? <FeedbackState tone="danger" autoFocus title={`Active ${title(kind).toLowerCase()} are temporarily unavailable`} action={<Button onClick={() => void load("active")}>Retry active {title(kind).toLowerCase()}</Button>}><p>{active.error.message} No protected rows are shown.</p></FeedbackState> : active.items.length === 0 && (!includeArchived || archived.items.length === 0) && !archived.loading && !archived.error && !search ? <EmptyState title={`No ${title(kind).toLowerCase()} yet`} action={canCreate ? <ActionLink variant="primary" href={`${base}/new`}>Add {singular(kind).toLowerCase()}</ActionLink> : undefined}><p>{includeArchived ? `No active or archived ${title(kind).toLowerCase()} are currently visible.` : `No active ${title(kind).toLowerCase()} are currently visible.`}</p></EmptyState> : search && visible.length === 0 ? <EmptyState title={`No loaded ${title(kind).toLowerCase()} match your search`} action={<Button onClick={() => { setSearchDraft(""); setSearch(""); }}>Clear search</Button>}><p>Clear the search or load more records. This search does not cover records that have not been loaded.</p></EmptyState> : <section className="cg-directory-results ds-responsive-record-list" aria-label={`Loaded ${title(kind).toLowerCase()}`}>
      <DataTable caption={`Loaded ${title(kind).toLowerCase()}. Active and archived feeds keep separate server ordering.`}><thead><tr><th scope="col">{singular(kind)}</th><th scope="col">Status</th><th scope="col">Updated</th><th scope="col">Actions</th></tr></thead><tbody>{visible.map(item => <tr key={item.id} className={item.status === "archived" ? "is-archived" : undefined}><th scope="row"><Link href={`${base}/${item.id}`}>{item.displayName}</Link></th><td><StatusBadge tone={item.status === "active" ? "success" : "neutral"}>{item.status === "active" ? "Active" : "Archived"}</StatusBadge></td><td><time dateTime={item.updatedAt}>{date(item.updatedAt)}</time></td><td><div className="cg-directory-actions"><ActionLink href={`${base}/${item.id}`}>View<span className="sr-only"> {item.displayName}</span></ActionLink>{item.capabilities.canEdit && <ActionLink href={`${base}/${item.id}/edit`}>Edit<span className="sr-only"> {item.displayName}</span></ActionLink>}<LifecycleAction workspaceId={workspaceId} kind={kind} item={item} onAuthorityLoss={clearProtected} onApplied={message => { setAnnouncement(message); void load("active"); if (includeArchived) void load("archived"); }} onStale={refresh}/></div></td></tr>)}</tbody></DataTable>
      <RecordCards label={`Loaded ${title(kind).toLowerCase()}`}>{visible.map(item => <RecordCard key={item.id} title={item.displayName} href={`${base}/${item.id}`} secondary={<StatusBadge tone={item.status === "active" ? "success" : "neutral"}>{item.status === "active" ? "Active" : "Archived"}</StatusBadge>} facts={[{label:"Updated",value:<time dateTime={item.updatedAt}>{date(item.updatedAt)}</time>}]} actions={<div className="cg-directory-actions"><ActionLink href={`${base}/${item.id}`}>View<span className="sr-only"> {item.displayName}</span></ActionLink>{item.capabilities.canEdit && <ActionLink href={`${base}/${item.id}/edit`}>Edit<span className="sr-only"> {item.displayName}</span></ActionLink>}<LifecycleAction workspaceId={workspaceId} kind={kind} item={item} onAuthorityLoss={clearProtected} onApplied={message => { setAnnouncement(message); void load("active"); if (includeArchived) void load("archived"); }} onStale={refresh}/></div>}/>)}</RecordCards>
