...
function mergeItems(current: Item[], incoming: Item[]) { const merged = new Map(current.map(item => [item.id, item])); for (const item of incoming) merged.set(item.id, item); return [...merged.values()]; }
  ...
  async function mutate() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (isAuthorityLoss(error)) { setOpen(false); onAuthorityLoss(error); return; }
...
export function CustomerGraphListPage({ workspaceId, kind }: { workspaceId: string; kind: CustomerGraphKind }) {
  ...
  const visible = [...active.items, ...(includeArchived ? archived.items : [])].filter(item => item.displayName.toLocaleLowerCase().includes(search.toLocaleLowerCase())),
    base = `/crm/${plural(kind)}`,
    Icon = kind === "company" ? Building2 : ContactRound,
    loaded = active.items.length + (includeArchived ? archived.items.length : 0);
