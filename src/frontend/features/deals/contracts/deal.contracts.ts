...
const stage = z.object({ stageId: uuid, pipelineId: uuid, code: clean(64), label: clean(100), outcomeClass: z.enum(["open", "won", "lost"]), sortKey: z.number().int().nonnegative(), defaultProbabilityBps: z.number().int().min(0).max(10000), version }).strict();
