...
export async function updateLead(pool: Pool, context: TenantContext, leadId: string, expectedVersion: number, raw: LeadInput) {
  ...
  return transaction(pool,async client=>{
    ...
    const privileged=current.actor_role_code==="owner"||current.actor_role_code==="admin";
    if(!privileged){
      // Members drive lifecycle on their own Leads. Assignment, outcome, visibility and
      // team sharing stay with owner/admin (canEditLead); stage moves need lead ownership
      // (canMoveLeadStage). This legacy path previously authorized on visibility alone.
      if(raw.ownerMembershipId!==undefined&&raw.ownerMembershipId!==current.owner_membership_id)throw new CrmError("permission_required",403);
      if(raw.status!==undefined&&raw.status!==current.status)throw new CrmError("permission_required",403);
      if(input.visibility!==current.visibility)throw new CrmError("permission_required",403);
      if(input.stageId!==current.stage_id&&current.owner_membership_id!==context.membershipId)throw new CrmError("permission_required",403);
      const currentTeams=(await client.query<{team_id:string}>(`select team_id from lead_visible_teams where workspace_id=$1 and lead_id=$2 order by team_id`,[context.workspaceId,leadId])).rows.map(row=>row.team_id);
      if(currentTeams.length!==input.teamIds.length||currentTeams.some((teamId,index)=>teamId!==input.teamIds[index]))throw new CrmError("permission_required",403);
    }
    const ownerId=await assertReferences(client,context,input);
    const nextStatus=privileged?input.status:current.status,nextOwnerId=privileged?ownerId:current.owner_membership_id,nextVisibility=privileged?input.visibility:current.visibility;
    const updated=(await client.query(`update leads set first_name=$3,last_name=$4,email_normalized=$5,email_display=$6,company=$7,phone=$8,source=$9,status=$10,stage_id=$11,owner_membership_id=$12,visibility=$13,version=version+1,updated_at=now() where id=$1 and workspace_id=$2 and version=$14 returning *`,[leadId,context.workspaceId,input.firstName,input.lastName,input.email,input.email.trim(),input.company,input.phone??null,input.source,nextStatus,input.stageId,nextOwnerId,nextVisibility,expectedVersion])).rows[0];
    ...
    const changes=[];if(current.stage_id!==input.stageId)changes.push("stage_changed");if(current.status!==nextStatus)changes.push("status_changed");
