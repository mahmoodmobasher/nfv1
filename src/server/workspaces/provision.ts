...
import { resolveSelectedCommercialPlan } from "../commercial/catalog";
...
export async function provisionWorkspace(pool:Pool,input:{userId:string;sessionId:string;name:string;idempotencyKey:string}){
  ...
  return transaction(pool,async client=>{
    ...
    let plan;
    try{plan=await resolveSelectedCommercialPlan(client,onboarding.selected_plan_code,onboarding.billing_cadence)}catch{throw new ProvisioningError("invalid_plan")}
    const start=new Date(),end=new Date(start.getTime()+plan.trialDays*86_400_000);
