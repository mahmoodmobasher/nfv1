...
export async function workspaceSummary(pool:Pool,userId:string){return(await pool.query(`select w.id,w.name,w.slug,w.plan_code,w.billing_cadence,w.trial_ends_at,m.id membership_id,r.code role from workspaces w join workspace_memberships m on m.workspace_id=w.id and m.user_id=$1 and m.status='active' join roles r on r.id=m.role_id and r.workspace_id=w.id where w.status='active' order by m.joined_at limit 1`,[userId])).rows[0]??null}
