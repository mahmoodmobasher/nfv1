...
const commercialCatalogVersion="2026-08-commercial-v1",commercialSeats:Record<string,number>={essentials:1,growth:5,scale:15};
function commercialPlan(plan:Record<string,unknown>|undefined,cadence:unknown){return Boolean(plan&&plan.catalog_version===commercialCatalogVersion&&typeof plan.code==="string"&&commercialSeats[plan.code]===plan.included_active_seats&&Array.isArray(plan.allowed_cadences)&&typeof cadence==="string"&&plan.allowed_cadences.includes(cadence)&&plan.currency_code==="USD"&&plan.billing_unit==="workspace_subscription"&&Number.isInteger(plan.monthly_price_cents)&&Number(plan.monthly_price_cents)>0&&Number.isInteger(plan.annual_monthly_equivalent_price_cents)&&Number(plan.annual_monthly_equivalent_price_cents)>0)}
...
export async function provisionWorkspace(pool:Pool,input:{userId:string;sessionId:string;name:string;idempotencyKey:string}){
  ...
  return transaction(pool,async client=>{
    ...
    const plan=(await client.query(`select * from plan_catalog_entries where code=$1 and status='active' and effective_from<=now()and(effective_to is null or effective_to>now()) order by effective_from desc,created_at desc,id desc limit 1`,[onboarding.selected_plan_code])).rows[0];
    if(!commercialPlan(plan,onboarding.billing_cadence))throw new ProvisioningError("invalid_plan");
