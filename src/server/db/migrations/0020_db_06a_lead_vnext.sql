...
BEGIN
  ...
  IF NEW.id IS DISTINCT FROM OLD.id OR NEW.workspace_id IS DISTINCT FROM OLD.workspace_id
    ...
    OR NEW.issue_code IS DISTINCT FROM OLD.issue_code OR NEW.created_at IS DISTINCT FROM OLD.created_at THEN
