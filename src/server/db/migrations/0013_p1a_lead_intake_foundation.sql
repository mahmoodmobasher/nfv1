...
CREATE TABLE "lead_intakes" (
	...
	CONSTRAINT "lead_intakes_actor_check" CHECK ("lead_intakes"."intake_channel"<>'manual' or "lead_intakes"."actor_membership_id" is not null),
...
BEGIN
  LOOP
    WITH batch AS (
      ...
      WHERE display_name IS NULL OR original_source_category IS NULL OR received_at IS NULL
    ...
    RAISE NOTICE 'p1a lead compatibility backfill batch: % rows', affected;
  END LOOP;
  IF EXISTS (SELECT 1 FROM leads WHERE display_name IS NULL OR original_source_category IS NULL OR received_at IS NULL)
     OR EXISTS (SELECT 1 FROM leads WHERE workspace_id IS NULL)
     OR EXISTS (SELECT 1 FROM leads WHERE status NOT IN ('open','won','lost')) THEN
    RAISE EXCEPTION 'p1a retained lead validation failed';
  END IF;
...
CREATE INDEX "contacts_workspace_name_idx" ON "contacts" USING btree ("workspace_id","display_name","company_id","id");--> statement-breakpoint
...
CREATE UNIQUE INDEX IF NOT EXISTS "lead_intakes_idempotency_uq" ON "lead_intakes" USING btree ("workspace_id","operation","idempotency_key");--> statement-breakpoint
...
ALTER TABLE "leads" ADD CONSTRAINT "leads_social_platform_check" CHECK (coalesce(("leads"."original_source_category"='social_media' and "leads"."original_source_platform" in ('tiktok','instagram','facebook','linkedin','x','youtube','other_social') and ("leads"."original_source_platform"<>'other_social' or length(btrim(coalesce("leads"."original_source_detail"->>'platform_context',''))) between 1 and 200)) or ("leads"."original_source_category"<>'social_media' and "leads"."original_source_platform" is null),false));--> statement-breakpoint
...
BEGIN
  IF NEW.id IS DISTINCT FROM OLD.id OR NEW.code IS DISTINCT FROM OLD.code THEN
    RAISE EXCEPTION 'lifecycle identity is immutable' USING ERRCODE='23514';
  END IF;
  RETURN NEW;
END $$;--> statement-breakpoint
CREATE TRIGGER lead_lifecycle_identity_immutable BEFORE UPDATE ON lead_lifecycle_definitions
FOR EACH ROW EXECUTE FUNCTION p1a_protect_lifecycle_identity();--> statement-breakpoint
...
BEGIN
  ...
  IF NEW.lifecycle_definition_id IS NULL THEN
    SELECT id INTO NEW.lifecycle_definition_id FROM lead_lifecycle_definitions WHERE code='new';
  END IF;
  RETURN NEW;
END $$;--> statement-breakpoint
CREATE TRIGGER leads_p1a_compatibility_defaults BEFORE INSERT ON leads
FOR EACH ROW EXECUTE FUNCTION p1a_lead_compatibility_defaults();--> statement-breakpoint
CREATE FUNCTION p1a_protect_lead_contract() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF OLD.lifecycle_definition_id IS NOT NULL AND NEW.lifecycle_definition_id IS NULL THEN
    RAISE EXCEPTION 'assigned lifecycle identity cannot be cleared' USING ERRCODE='23514';
  END IF;
  IF NEW.original_source_category IS DISTINCT FROM OLD.original_source_category
     OR NEW.original_source_platform IS DISTINCT FROM OLD.original_source_platform
     OR NEW.original_source_medium IS DISTINCT FROM OLD.original_source_medium
     OR NEW.original_source_detail IS DISTINCT FROM OLD.original_source_detail
     OR NEW.original_campaign_context IS DISTINCT FROM OLD.original_campaign_context
     OR NEW.attribution_contract_version IS DISTINCT FROM OLD.attribution_contract_version
     OR NEW.intake_channel IS DISTINCT FROM OLD.intake_channel
     OR NEW.received_at IS DISTINCT FROM OLD.received_at THEN
    RAISE EXCEPTION 'original lead attribution is immutable' USING ERRCODE='23514';
  END IF;
  RETURN NEW;
END $$;--> statement-breakpoint
CREATE TRIGGER leads_p1a_contract_immutable BEFORE UPDATE ON leads
FOR EACH ROW EXECUTE FUNCTION p1a_protect_lead_contract();--> statement-breakpoint
...
BEGIN
  IF NEW.state='committed' THEN
    SELECT * INTO linked_lead FROM leads WHERE workspace_id=NEW.workspace_id AND id=NEW.lead_id;
    IF NOT FOUND OR linked_lead.original_source_category IS DISTINCT FROM NEW.source_category
       OR linked_lead.original_source_platform IS DISTINCT FROM NEW.source_platform
       OR linked_lead.original_source_medium IS DISTINCT FROM NEW.source_medium
       OR linked_lead.original_source_detail IS DISTINCT FROM NEW.source_detail
       OR linked_lead.original_campaign_context IS DISTINCT FROM NEW.campaign_context
       OR linked_lead.attribution_contract_version IS DISTINCT FROM NEW.attribution_contract_version
       OR linked_lead.intake_channel IS DISTINCT FROM NEW.intake_channel
       OR linked_lead.lifecycle_definition_id IS DISTINCT FROM '00000000-0000-4000-8000-000000000001'::uuid THEN
      RAISE EXCEPTION 'committed intake must match immutable P1A lead contract' USING ERRCODE='23514';
    END IF;
  END IF;
  RETURN NEW;
END $$;--> statement-breakpoint
CREATE TRIGGER lead_intakes_committed_contract_check BEFORE INSERT OR UPDATE ON lead_intakes
FOR EACH ROW EXECUTE FUNCTION p1a_validate_committed_intake();--> statement-breakpoint
CREATE FUNCTION p1a_validate_review_lineage() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM lead_intakes WHERE workspace_id=NEW.workspace_id AND id=NEW.intake_id AND lead_id=NEW.lead_id AND state='committed') THEN
    RAISE EXCEPTION 'review must match one committed intake lead' USING ERRCODE='23514';
  END IF;
  RETURN NEW;
END $$;--> statement-breakpoint
CREATE TRIGGER lead_identity_reviews_lineage_check BEFORE INSERT OR UPDATE OF workspace_id,intake_id,lead_id ON lead_identity_reviews
FOR EACH ROW EXECUTE FUNCTION p1a_validate_review_lineage();--> statement-breakpoint
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM lead_identity_reviews r JOIN lead_intakes i ON i.workspace_id=r.workspace_id AND i.id=r.intake_id
                 WHERE r.workspace_id=NEW.workspace_id AND r.id=NEW.review_id AND r.intake_id=NEW.intake_id
                   AND i.version=NEW.expected_intake_version) THEN
    RAISE EXCEPTION 'decision review/intake lineage or expected intake version mismatch' USING ERRCODE='23514';
  END IF;
  SELECT l.version,r.version INTO current_lead_version,current_review_version
    FROM lead_identity_reviews r JOIN leads l ON l.workspace_id=r.workspace_id AND l.id=r.lead_id
    WHERE r.workspace_id=NEW.workspace_id AND r.id=NEW.review_id;
  IF current_lead_version IS DISTINCT FROM NEW.expected_lead_version OR current_review_version IS DISTINCT FROM NEW.expected_review_version THEN
    RAISE EXCEPTION 'stale Lead or review version' USING ERRCODE='23514';
  END IF;
  IF NEW.supersedes_decision_id IS NOT NULL AND NOT EXISTS
     (SELECT 1 FROM lead_identity_decisions WHERE workspace_id=NEW.workspace_id AND intake_id=NEW.intake_id AND id=NEW.supersedes_decision_id) THEN
    RAISE EXCEPTION 'invalid decision supersession' USING ERRCODE='23514';
  END IF;
  IF NEW.contact_action='link' THEN
    SELECT contact_id,target_version INTO candidate_target,candidate_version FROM lead_identity_candidates
      WHERE workspace_id=NEW.workspace_id AND review_id=NEW.review_id AND id=NEW.contact_candidate_id;
    SELECT version INTO current_target_version FROM contacts WHERE workspace_id=NEW.workspace_id AND id=NEW.contact_id;
    IF candidate_target IS DISTINCT FROM NEW.contact_id OR candidate_version IS DISTINCT FROM NEW.contact_target_version
       OR current_target_version IS DISTINCT FROM NEW.contact_target_version THEN
      RAISE EXCEPTION 'stale or mismatched Contact candidate' USING ERRCODE='23514'; END IF;
  END IF;
  IF NEW.company_action='link' THEN
    SELECT company_id,target_version INTO candidate_target,candidate_version FROM lead_identity_candidates
      WHERE workspace_id=NEW.workspace_id AND review_id=NEW.review_id AND id=NEW.company_candidate_id;
    SELECT version INTO current_target_version FROM companies WHERE workspace_id=NEW.workspace_id AND id=NEW.company_id;
    IF candidate_target IS DISTINCT FROM NEW.company_id OR candidate_version IS DISTINCT FROM NEW.company_target_version
       OR current_target_version IS DISTINCT FROM NEW.company_target_version THEN
      RAISE EXCEPTION 'stale or mismatched Company candidate' USING ERRCODE='23514'; END IF;
  END IF;
  RETURN NEW;
END $$;--> statement-breakpoint
CREATE TRIGGER lead_identity_decisions_lineage_check BEFORE INSERT ON lead_identity_decisions
FOR EACH ROW EXECUTE FUNCTION p1a_validate_decision_lineage();--> statement-breakpoint
CREATE FUNCTION p1a_validate_decision_head() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE prior uuid;
BEGIN
  SELECT supersedes_decision_id INTO prior FROM lead_identity_decisions
    WHERE workspace_id=NEW.workspace_id AND intake_id=NEW.intake_id AND id=NEW.decision_id;
  IF NOT FOUND OR (TG_OP='INSERT' AND prior IS NOT NULL)
     OR (TG_OP='UPDATE' AND (prior IS DISTINCT FROM OLD.decision_id OR NEW.version<>OLD.version+1)) THEN
    RAISE EXCEPTION 'effective decision head requires contiguous immutable supersession' USING ERRCODE='23514';
  END IF;
  RETURN NEW;
END $$;--> statement-breakpoint
CREATE TRIGGER lead_identity_decision_heads_lineage_check BEFORE INSERT OR UPDATE ON lead_identity_decision_heads
FOR EACH ROW EXECUTE FUNCTION p1a_validate_decision_head();--> statement-breakpoint
CREATE FUNCTION p1a_protect_committed_intake() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF OLD.state='committed' AND (NEW.workspace_id,NEW.operation,NEW.intake_channel,NEW.idempotency_key,NEW.actor_membership_id,
    NEW.request_hash,NEW.contract_version,NEW.normalization_version,NEW.attribution_contract_version,NEW.source_category,
    NEW.source_platform,NEW.source_medium,NEW.source_detail,NEW.campaign_context,NEW.state,NEW.lead_id,NEW.outcome)
    IS DISTINCT FROM (OLD.workspace_id,OLD.operation,OLD.intake_channel,OLD.idempotency_key,OLD.actor_membership_id,
    OLD.request_hash,OLD.contract_version,OLD.normalization_version,OLD.attribution_contract_version,OLD.source_category,
    OLD.source_platform,OLD.source_medium,OLD.source_detail,OLD.campaign_context,OLD.state,OLD.lead_id,OLD.outcome) THEN
    RAISE EXCEPTION 'committed intake authority is immutable' USING ERRCODE='23514';
  END IF;
  RETURN NEW;
END $$;--> statement-breakpoint
CREATE TRIGGER lead_intakes_committed_immutable BEFORE UPDATE ON lead_intakes
FOR EACH ROW EXECUTE FUNCTION p1a_protect_committed_intake();--> statement-breakpoint
CREATE FUNCTION p1a_reject_lineage_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'P1A lineage is append-only' USING ERRCODE='23514'; END $$;--> statement-breakpoint
