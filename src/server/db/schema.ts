...
export const leads = pgTable(
  "leads",
  {
    ...
    lifecycleReopenCount: integer("lifecycle_reopen_count").notNull().default(0),
  ...
  (table) => [
    ...
    check("leads_current_source_platform_check", sql`coalesce((${table.source}='social_media' and ${table.sourcePlatform} in ('tiktok','instagram','facebook','linkedin','x','youtube','other_social')) or (${table.source}<>'social_media' and ${table.sourcePlatform} is null),false)`),
    ...
    check("leads_status_source_check", sql`${table.statusSource} in ('system','manual')`),
    check("leads_disqualification_reason_check", sql`${table.disqualificationReason} is null or ${table.disqualificationReason} in ('not_a_fit','no_response','duplicate','bad_data','no_budget','lost_to_competitor','other')`),
    check("leads_disqualification_note_check", sql`(${table.disqualificationNote} is null or char_length(btrim(${table.disqualificationNote})) between 1 and 1000) and (${table.disqualificationReason} is distinct from 'other' or ${table.disqualificationNote} is not null) and (${table.disqualificationReason} is not null or ${table.disqualificationNote} is null)`),
    check("leads_lifecycle_reopen_count_check", sql`${table.lifecycleReopenCount} >= 0`),
