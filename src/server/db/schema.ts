...
export const contacts = pgTable(
  ...
  (table) => [
    ...
    index("contacts_workspace_name_idx").on(table.workspaceId, table.displayName, table.companyId, table.id),
...
export const leads = pgTable(
  ...
  (table) => [
    ...
    check("leads_phone_pair_check", sql`(${table.phone} is null and ${table.phoneNormalized} is null and ${table.phoneCountryCodeUsed} is null) or (${table.phone} is not null and ${table.phoneNormalized} is not null and length(btrim(${table.phoneCountryCodeUsed})) between 2 and 16)`),
...
export const leadIntakes = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("lead_intakes_idempotency_uq").on(table.workspaceId, table.operation, table.idempotencyKey),
    ...
    check("lead_intakes_actor_check", sql`${table.intakeChannel}<>'manual' or ${table.actorMembershipId} is not null`),
