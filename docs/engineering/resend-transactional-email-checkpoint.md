...
Status: **implementation and local validation complete; UAT deployment blocked on protected credentials and an approved test inbox**
...
- The approved sender boundary is `mail.nexaflowsystems.com`; the intended sender is `NexaFlow accounts <accounts@mail.nexaflowsystems.com>`. `EMAIL_REPLY_TO` remains unset unless Product approves a monitored address.
...
UAT was not changed. Deployment must not begin until both are supplied through an approved protected channel:

1. A new restricted Resend API key that is permitted to send from `mail.nexaflowsystems.com`.
2. An approved real inbox for end-to-end registration verification, password recovery, and invitation delivery.

The key must be written only to the host-owned protected application environment, mode `0600`, without printing it. A deployment without the key intentionally fails readiness. The `example.test` cohort is synthetic and cannot prove public email delivery.
