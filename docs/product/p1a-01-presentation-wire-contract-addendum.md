...
Errors are `{error:{code,message,retryable,reconciliation:{required,action}},requestId}`. Messages are stable and non-sensitive. Reconciliation action is `none`, `refetch_identity_review`, or `retry_same_request`. A 409 may include an identity-review `nextView` only after the route already established a safe Lead route context; authentication, permission, and not-found failures never contain it. Responses use `Cache-Control: private, no-store, max-age=0`, `Pragma: no-cache`, and `Vary: cookie`.
